...
import global_planners.user.layered_a_star_port_priority_aware_planner
...
def create_cmd_parser(prog=None):
    ...
    # 这里的全局规划器会被挂到每个 agent 上。
    # 对于本项目的改进版本，我们刻意保持“一轮改进 = 一个独立脚本 = 一个独立类”，
    # 这样在 meeting 和最终报告里可以直接从文件级别展示每轮算法改了什么。
    general_control.add_argument('--gp', action='store', type=str, dest='global_planner', help='Set the global planners for all agents, choices include: LayeredAStar | LayeredAStarCollisionAware | LayeredAStarReservationAware | LayeredAStarQueueAware | LayeredAStarPortPriorityAware | LayeredAStarBaselineTrafficAware | RRTStar | MARRTStar | INashRRT')
    ...
    # task manager 作为补充分支单独切换，便于把“怎么走”和“先去哪里”分开比较。
    general_control.add_argument('--tm', action='store', type=str, dest='task_manager', help='Set the task manager, choices include: NaiveTaskManager | CongestionAwareTaskManager')
    general_control.add_argument('--record-tag', action='store', type=str, dest='record_tag', help='Optional tag used in structured experiment output files')
    general_control.add_argument('--record-dir', action='store', type=str, dest='record_dir', help='Optional directory for structured experiment output')
    general_control.add_argument('--stuck-recovery', action='store_true', dest='stuck_recovery', help='Enable the v5 queue/contact recovery hooks instead of the clean baseline runtime')
    general_control.add_argument('--stuck-recovery-threshold', action='store', type=int, default=10, dest='stuck_recovery_threshold', help='Persistent contact steps required before the recovery hooks trigger')
...
def process_global_planner_cmd(global_planner_name):
    ...
    # 先在已经导入的 planner 子类里查找。
    # 由于每轮改进都拆成独立脚本，只要模块被导入，这里就能按类名直接切换版本。
...
def process_task_manager_cmd(task_manager_name):
    if not task_manager_name:
        return None

    # task manager 也保持“一个版本一个类”的方式，便于补充分支单独展示。
    def iter_task_manager_classes(base_class):
        for subclass in base_class.__subclasses__():
            yield subclass
            for nested in iter_task_manager_classes(subclass):
                yield nested

    for task_manager_class in iter_task_manager_classes(TaskMananger):
        if task_manager_name == task_manager_class.__name__:
            return task_manager_class
    module_name, class_name = convert_planner_name_by_uppercase(str(task_manager_name))
    manager_class = dynamic_importer(module_name, class_name)
    if manager_class and TaskMananger.__subclasscheck__(manager_class):
