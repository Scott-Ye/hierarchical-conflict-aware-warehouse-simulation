...
class INashRRT(MultiAgentPlanner):
    ...
    def compute_path(self):
        ...
        if INashRRT.VISUAL:
            for agent in list(agents_abstraction_dict.values()):
        ...
        agents_best_edge_path_dict = {aid:[] for aid in list(agents_abstraction_dict.keys())} # <aid, agent's edge list which forms best reponse>
        ...
        while not terminated:
            ...
            for agent in list(agents_abstraction_dict.values()):
                ...
                for temp_agent in list(agents_abstraction_dict.values()):
        ...
        if INashRRT.VISUAL:
            for _, path in list(agents_point_path_dict.items()):
    ...
    def terminate(self, time_elapsed, num_each_agent_states, agents_best_edge_path_dict):
        ...
        if all([len(path) for path in list(agents_best_edge_path_dict.values())]):
            return True
    ...
    def extract_solution_paths_from_edges(self, agents_best_edge_path_dict):
        ...
        agents_point_path_dict = {aid:[] for aid in list(agents_best_edge_path_dict.keys())}
        agents_cost_dict = {aid:0 for aid in list(agents_best_edge_path_dict.keys())}
        for aid, edges in list(agents_best_edge_path_dict.items()):
            if len(edges) == 0:
                continue
            (edge,_,_) = edges[0]
    ...
    def multiple_agents_path_free(self, agent_id, edge_timestamp_path, agents_best_edge_path_dict, agents_abstraction_dict):
        ...
        if self.steer_func == self.straight_line_steer: # check timestamp
            for (edge, start_t, end_t) in edge_timestamp_path:
                ...
                for other_edge_t_path in list(temp_agents_best_edge_path_dict.values()):
