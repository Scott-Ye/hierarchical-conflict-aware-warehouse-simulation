...
# """
    ...
    def setup_simple_queue_area(self, port, gap_between_ports):
        ...
        y = port.location.y
        # 顶边 loading port 的 queue 若与 port 同行铺开，
        # source exit 轨迹会与 follower 队列共线，容易在离港时擦碰。
        if getattr(port, "port_type", None) == "loading" and y <= port.dimension[1]:
            y = min(self.height_in_meters - port.dimension[1], y + port.dimension[1])
        # 底边 port 的 queue 若仍贴着最外侧边界，slot 中心会落到墙边，
        # agent 在 QUEUING 时容易因为物理半径与墙接触。
        if y >= self.height_in_meters - port.dimension[1]:
            y = max(0, y - port.dimension[1])
