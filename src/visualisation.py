...
def is_key(event, key_const, key_char):
    if event.type != KEYDOWN:
        return False
    if event.key == key_const:
        return True
    return getattr(event, "unicode", "").lower() == key_char

"""only for debug"""
def draw_everything(polygon, body, fixture, screen, object_box, PPM, reference_point, is_sensor):
    vertices = [as_int_point(((body.transform * v)[0] * PPM + reference_point[0],
                              (body.transform * v)[1] * PPM + reference_point[1]))
...
def draw_agent(polygon, body, fixture, screen, agent, PPM, workspace_dimension, reference_point, show_agent_details, is_sensor, debug_color = (125, 125, 125, 255)):
    vertices = [as_int_point(((body.transform * v)[0] * PPM + reference_point[0],
                              (body.transform * v)[1] * PPM + reference_point[1]))
                for v in polygon.vertices]
    centroid = as_int_point((body.position[0] * PPM + reference_point[0],
                             body.position[1] * PPM + reference_point[1]))
    arrow_end = as_int_point((centroid[0] + body.linearVelocity[0] * PPM / 2.0,
                              centroid[1] + body.linearVelocity[1] * PPM / 2.0))
    ...
    if is_sensor:
        ...
        pygame.draw.circle(screen, (0, 255, 0, 255), arrow_end, 4)
    ...
    if show_agent_details:
        def readable_agent_state(agent):
            if getattr(agent, "stopping_active", False):
                base_state = getattr(getattr(agent, "stopping_base_state", None), "name", None)
                if base_state == "LOADING":
                    task_type = getattr(getattr(agent, "task", None), "type", None)
                    if task_type == TaskType.GO_TO_UNLOADING_PORT:
                        base_state = "UNLOADING"
                    elif task_type == TaskType.GO_TO_LOADING_PORT:
                        base_state = "LOADING"
                if base_state:
                    return "STOPPING({})".format(base_state)
                return "STOPPING"
            state_name = str(agent.state)[11:]
        ...
        details_font=pygame.font.SysFont("Calibri",int(14*PPM/30.0))
        details_font.set_bold(True)
        destination_text = "None"
        if agent.destination_location:
            destination_text = "("+ str(int(agent.destination_location.x))+","+str(int(agent.destination_location.y))+")"
            pygame.draw.circle(screen, agent_color, rectify_vertex_position(agent.destination_location), 7, 0)

        detail_text = (
            "Agent{0}:({1},{2}) DEST:{3} STATE:{4} TASK:{5}".format(
                agent.id,
                int(agent.position.x),
                int(agent.position.y),
                destination_text,
                readable_agent_state(agent),
                readable_agent_task(agent),
            )
        )
        text_obj2=details_font.render(detail_text, True, (0,0,0))
        text_pos2 = text_obj2.get_rect()
        workspace_height = workspace_dimension[1]
        max_rows_per_column = 6
        column_index = agent.id // max_rows_per_column
        row_index = agent.id % max_rows_per_column
        column_width = int(PPM * 11.5)
        line_height = max(details_font.get_height() + 2, int(PPM * 0.55))
        text_pos2.topleft=(
            int(PPM + column_width * column_index + reference_point[0]),
            int(workspace_height + PPM * 4 + line_height * row_index + reference_point[1])
        )
...
class Visualisation:
    def __init__(self, simulator, config_data):
        ...
        ref = self.pixels_per_meter / self.grids_num_per_meter * 2
        self.reference_point = (int(ref), int(ref))
    ...
    def run(self):
        ...
        if self._should_auto_resume_debug_session():
            pause_simulator = False
        show_agent_details = self.simulator.agent_details # default False
        DEBUG_MODE = False
        show_sensor_range = False
        _dbg_last_loop_wall = pygame.time.get_ticks()
        if self._should_auto_resume_debug_session():
            # #region debug-point D:gui-auto-resume
            self._debug_gui_event(
                "D",
                "visualisation.py:run:init",
                "[DEBUG] GUI auto-resumed for debug session",
                {
                    "pause_simulator": pause_simulator,
                    "step": getattr(self.simulator, "step_counter", None),
                    "sim_time": round(self.simulator.get_simulator_time(), 3),
                },
            )
            # #endregion
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
                elif is_key(event, pygame.K_q, 'q'):
                        running = False
                        break
                elif is_key(event, pygame.K_d, 'd'):
                    show_agent_details = not show_agent_details
                elif is_key(event, pygame.K_s, 's'):
                    show_sensor_range = not show_sensor_range
                elif is_key(event, pygame.K_g, 'g'):
                    show_grid = not show_grid
                elif is_key(event, pygame.K_p, 'p'):
                    pause_simulator = not pause_simulator
                    # #region debug-point D:gui-toggle-pause
                    self._debug_gui_event(
                        "D",
                        "visualisation.py:run",
                        "[DEBUG] GUI toggled pause state",
                        {
                            "pause_simulator": pause_simulator,
                            "step": getattr(self.simulator, "step_counter", None),
                            "sim_time": round(self.simulator.get_simulator_time(), 3),
                    ...
                    # #endregion
                elif is_key(event, pygame.K_r, 'r'):
                    show_control_range = not show_control_range
                elif is_key(event, pygame.K_b, 'b'):
            ...
            text_pos1.center=(int(self.screen_width/2), int(self.workspace_height+self.pixels_per_meter+self.reference_point[1]))
            ...
            text_obj2=font1.render(
                ...
                True, (0,0,0))
            text_pos2=text_obj2.get_rect()
            text_pos2.center=(int(self.screen_width/2), int(self.workspace_height + 2*self.pixels_per_meter + self.reference_point[1]))
            screen.blit(text_obj2,text_pos2)
            try:
                ...
                text_pos3.center=(int(self.screen_width/2), int(self.workspace_height+3*self.pixels_per_meter+self.reference_point[1]))
            ...
            while pause_simulator:
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        pause_simulator = not pause_simulator
                        # #region debug-point D:gui-toggle-pause-while-paused
                        self._debug_gui_event(
                            "D",
                            "visualisation.py:run:pause-loop",
                            "[DEBUG] GUI toggled pause state inside pause loop",
                            {
                                "pause_simulator": pause_simulator,
                                "step": getattr(self.simulator, "step_counter", None),
                                "sim_time": round(self.simulator.get_simulator_time(), 3),
                ...
                if update:
                    self.draw_everything(screen, show_grid, show_control_range, show_agent, show_agent_details, show_sensor_range)
                    pygame.display.flip()
            _dbg_step_start = pygame.time.get_ticks()
            self.simulator.step()
            _dbg_now = pygame.time.get_ticks()
            if (
                ((_dbg_now - _dbg_last_loop_wall) >= 200 or (_dbg_now - _dbg_step_start) >= 200)
                and getattr(self.simulator, "step_counter", 0) % 10 == 0
            ):
                # #region debug-point D:gui-frame-timing
                self._debug_gui_event(
                    "D",
                    "visualisation.py:run:frame",
                    "[DEBUG] GUI frame timing sample",
                    {
                        "step": getattr(self.simulator, "step_counter", None),
                        "sim_time": round(self.simulator.get_simulator_time(), 3),
                        "loop_wall_ms": int(_dbg_now - _dbg_last_loop_wall),
                        "step_wall_ms": int(_dbg_now - _dbg_step_start),
                        "pause_simulator": pause_simulator,
                    },
                )
                # #endregion
            _dbg_last_loop_wall = _dbg_now
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if clicked_ports != None and event.button == 3:
                for port in clicked_ports:
                    ...
                    print("port type", getattr(port, "port_type", "unknown"))
    ...
    def draw_grid(self, show_grid, screen, color):
        ...
        if show_grid:
            num_rows = int((self.workspace_height - self.pixels_per_meter) // pixels_per_grid)
            num_cols = int((self.workspace_width - self.pixels_per_meter) // pixels_per_grid)
            for i in range(num_rows):
                start = as_int_point((self.reference_point[0], i * pixels_per_grid + self.reference_point[1]))
                end = as_int_point((self.workspace_width - self.pixels_per_meter + self.reference_point[0],
                    i * pixels_per_grid + self.reference_point[1]))
                pygame.draw.line(screen, color, start, end, 1)
            for j in range(num_cols):
                start = as_int_point((j * pixels_per_grid + self.reference_point[0], self.reference_point[1]))
                end = as_int_point((j * pixels_per_grid + self.reference_point[0],
                    self.workspace_height - self.pixels_per_meter +  self.reference_point[1]))
                pygame.draw.line(screen, color, start, end, 1)
        else:
            pass
    ...
    def draw_raycast(self, screen):
        ...
        for points in points_list:
            line = [as_int_point((points[0][0]  * self.pixels_per_meter + self.reference_point[0], points[0][1]* self.pixels_per_meter + self.reference_point[1])), 
            as_int_point((points[1][0] * self.pixels_per_meter + self.reference_point[0], points[1][1] *self.pixels_per_meter + self.reference_point[1]))]
    ...
    def draw_workspace(self, screen):
        color = (0, 0, 0, 255)
        workspace_point_list = [self.reference_point, as_int_point((self.reference_point[0], self.workspace_height - self.pixels_per_meter + self.reference_point[1])),
        as_int_point((self.workspace_width - self.pixels_per_meter + self.reference_point[0], self.workspace_height - self.pixels_per_meter + self.reference_point[1])),
        as_int_point((self.workspace_width - self.pixels_per_meter + self.reference_point[0], self.reference_point[1]))]
    ...
    def draw_control_range(self, screen, port, show_control_range):
        ...
        if show_control_range:
            pygame.draw.circle(screen,
                color1,
                as_int_point((self.meter_to_pixel(port.queue.region.x) + self.reference_point[0], self.meter_to_pixel(port.queue.region.y) + self.reference_point[1])),
                self.meter_to_pixel(port.control_range),
                2)
            pygame.draw.circle(screen,
                color2,
                as_int_point(( self.meter_to_pixel(port.queue.region.x)+self.meter_to_pixel(port.queue.region.width)+self.reference_point[0],
    ...
    def draw_rectangle(self, screen, x, y, depth, width, color, fill = 0):
        pygame.draw.rect(screen, color, Rect(int(x + self.reference_point[0]), int(y + self.reference_point[1]), int(depth), int(width)), fill)
    ...
    def draw_port_id(self, screen, port):
        ...
        font.set_bold(True)
        port_prefix = "P"
        if getattr(port, "port_type", None) == "loading":
            port_prefix = "L"
        elif getattr(port, "port_type", None) == "unloading":
            port_prefix = "U"
        identifier = "{}{}".format(port_prefix, port.identifier)
    ...
    def __terminal_print(self):
        print ("")
        print ("--------------------------------------------------------------------------")
        print ("Green ports are L*, Red ports are U*")
        print ("--------------------------------------------------------------------------")
